...
class Download():
    ...
    garmin_connect_hrv_monitoring_url = "/hrv-service/hrv/{start}"
    garmin_connect_hrv_url = "/hrv-service/hrv/daily/{start}/{end}"
    ...
    def __get_hrv_day(self, directory, day, overwite=False):
        ...
        try:
            self.save_json_to_file(json_filename, self.garth.connectapi(self.garmin_connect_hrv_url.format(start=date_str, end=date_str)), overwite)
        except GarthHTTPError as e:
            root_logger.error("Exception getting daily summary: %s", e)
            
    def __get_hrv_monitoring_day(self, directory, day, overwite=False):
        root_logger.info("Checking hrv monitoring: %s overwite %r", day, overwite)
        date_str = day.strftime('%Y-%m-%d')
        json_filename = f'{directory}/hrv_monitoring_{date_str}'
        try:
            self.save_json_to_file(json_filename, self.garth.connectapi(self.garmin_connect_hrv_monitoring_url.format(start=date_str)), overwite)
        except GarthHTTPError as e:
            root_logger.error("Exception getting daily summary: %s", e)
    ...
    def get_hrv_monitoring(self, directory, date, days, overwite):
        """Download the hrv monitoring data from Garmin Connect and save to a JSON file."""
        root_logger.info("Getting hrv monitoring: %s (%d)", date, days)
        self.__get_stat(self.__get_hrv_monitoring_day, directory, date, days, overwite)
